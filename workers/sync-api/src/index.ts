...
// Vault routes
import { listVaults, createVault, getVault, deleteVault, getVaultKey, putVaultKey } from './routes/vaults';
// Sync routes
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (method === 'DELETE' && deviceMatch) {
        response = await revokeDevice(request, env, userId, deviceId, deviceMatch[1]);
        return addCorsHeaders(response, origin);
      }
