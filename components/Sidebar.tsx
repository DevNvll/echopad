    ...
    <div>
      ...
      >
        ...
        <Hash size={16} className={clsx(
          "shrink-0",
          isActive ? "text-brand" : "text-textMuted/60 group-hover:text-textMuted"
        )} />
        
        <span className="truncate leading-none flex-1 text-left">{notebook.name}</span>
...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const renderPinnedNotebookItem = (notebook: Notebook) => (
    ...
    >
      <Hash size={16} className={clsx(
        "shrink-0",
        activeNotebook === notebook.relativePath ? "text-brand" : "text-textMuted/60 group-hover:text-textMuted"
      )} />
